...
class TBAccountService {
    ...
    static func fetchQRCode() async throws -> String {
        ...
        let result = try await req.receiveOrThrow(
            ...
            reqBody: try! JSONSerialization.data(
                withJSONObject: ["app_id": "2", "device": UserDefaults.configGetConfig(forKey: TBData.DEVICE_ID, def: "")]
            ))
    ...
    static func queryCodeState(ticket: String) async throws -> Data {
        ...
        let result = try await req.receiveOrThrow(
            isPost: true,
            reqBody: try! JSONSerialization.data(
                withJSONObject: ["app_id": "2", "device": UserDefaults.configGetConfig(forKey: TBData.DEVICE_ID, def: ""), "ticket": ticket]
            ))
