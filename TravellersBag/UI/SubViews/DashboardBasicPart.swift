...
extension DashboardView {
    struct DashboardBasicPart: View {
        let content: JSON?
        let checkFileExist: (String) -> Void
        let gameUid: String
        ...
        var body: some View {
            if let content = content {
                ...
                VStack {
                    ...
                    Button("dashboard.fetch", action: { checkFileExist(gameUid) }).buttonStyle(BorderedProminentButtonStyle())
                }
                .padding()
                .background(RoundedRectangle(cornerRadius: 8, style: .continuous).fill(BackgroundStyle()).shadow(radius: 4, y: 4))
            }
            ...
            .sheet(isPresented: $showDetail, content: {
                ...
                .toolbar {
                    ToolbarItem(placement: .confirmationAction, content: {
                        Button("def.confirm", action: { showDetail = false })
                    })
                }
            })
        }
