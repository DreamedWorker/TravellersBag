...
struct ResourceDownload : View {
    ...
    var body: some View {
        ScrollView {
            ...
            Spacer()
            HStack(spacing: 16) {
                Button(action: { useBundleAlert = true }, label: { Text("wizard.resource.useBuiltIn").padding(4) })
                ...
                if model.uiState.canGoNext || UserDefaults.configGetConfig(forKey: "staticWizardDownloaded", def: false) {
                    Button(
                        action: { navigator(1) },
                        label: { Text("wizard.resource.next").padding(4) }
