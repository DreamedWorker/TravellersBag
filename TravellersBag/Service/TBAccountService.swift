...
class TBAccountService {
    ...
    static func fetchQRCode() async throws -> String {
        ...
        let result = try await req.receiveOrThrow(
            ...
            reqBody: try! JSONSerialization.data(
                withJSONObject: ["app_id": "2", "device": UserDefaults.configGetConfig(forKey: TBConfigKeys.DEVICE_ID, def: "")]
            ))
    ...
    static func queryCodeState(ticket: String) async throws -> Data {
        var req = URLRequest(url: URL(string: ApiEndpoints.shared.getQueryQRState())!)
        let result = try await req.receiveOrThrow(
            isPost: true,
            reqBody: try! JSONSerialization.data(
                withJSONObject: ["app_id": "2", "device": UserDefaults.configGetConfig(forKey: TBConfigKeys.DEVICE_ID, def: ""), "ticket": ticket]
            ))
        if result["stat"].stringValue != "Confirmed" {
            throw NSError(
                ...
                code: -106,
                userInfo: [
                    NSLocalizedDescriptionKey: String.localizedStringWithFormat(NSLocalizedString("account.error.qrcode_state_err", comment: ""), result["stat"].stringValue)
                ])
        }
