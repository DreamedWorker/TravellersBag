...
class TBEnv {
    ...
    func checkEnvironment(){
        if UserDefaultHelper.shared.getValue(forKey: "deviceID", def: "") == "" {
            UserDefaultHelper.shared.setValue(forKey: "deviceID", value: UUID().uuidString.lowercased())
        }
        if UserDefaultHelper.shared.getValue(forKey: "bbsDeviceID", def: "") == "" {
            UserDefaultHelper.shared.setValue(forKey: "bbsDeviceID", value: UUID().uuidString.lowercased())
        }
    }
    ...
    func checkFigurePointer() async {
        if UserDefaultHelper.shared.getValue(forKey: "deviceFp", def: "") != "" {
            return
        } else {
    ...
    func updateDeviceFp() async {
        ...
        let allDic: [String:Any] = [
            ...
            "app_name": "bbs_cn",
            "bbs_device_id": UserDefaultHelper.shared.getValue(forKey: TBEnv.BBS_DEVICE_ID, def: ""),
        ...
        do {
            ...
            if result["code"].intValue == 200 {
                UserDefaultHelper.shared.setValue(forKey: "deviceFp", value: result["device_fp"].stringValue)
                print("使用了已经过验证的fp")
            ...
            UserDefaultHelper.shared.setValue(forKey: "deviceFp", value: getLowerHexString(length: 13))
