...
extension URLRequest {
    ...
    mutating func receiveOrBlackData(isPost: Bool = false, reqBody: Data? = nil) async -> Data {
        if isPost { // 设定请求方法 已知水社只需要下面两个方法就够了
            self.httpMethod = "POST"
            self.httpBody = reqBody
        } else {
        ...
        do {
            ...
            if json["retcode"].intValue == 0 {
                ...
                return try! JSONSerialization.data(withJSONObject: ["ProgramError": "\(json["message"].string ?? "未知错误")"])
            }
        } catch {
            return try! JSONSerialization.data(withJSONObject: ["ProgramError": "未知错误"])
        }
    }
...
extension URLRequest {
    ...
    mutating func setDeviceInfoHeaders() {
        self.setValue(PreferenceMgr.default.getValue(key: TBData.DEVICE_FP, def: ""), forHTTPHeaderField: "x-rpc-device_fp")
        self.setValue("iPhone15,1", forHTTPHeaderField: "x-rpc-device_name")
        self.setValue(PreferenceMgr.default.getValue(key: TBData.DEVICE_ID, def: ""), forHTTPHeaderField: "x-rpc-device_id")
        self.setValue("16_0", forHTTPHeaderField: "x-rpc-sys_version")
    }
    
    /// 设置动态密钥
    mutating func setDS(
        version: DynamicSecret.SaltVersion,
        type: DynamicSecret.SaltType,
        body: String = "", q: String = "", include: Bool = true
    ) {
        self.setValue(
            DynamicSecret.getDynamicSecret(
                version: version, saltType: type,
                includeChars: include, query: q, body: body
