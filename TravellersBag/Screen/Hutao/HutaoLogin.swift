...
private class HutaoLoginModel: ObservableObject {
    ...
    func tryLogin() async {
        do {
            ...
            TBDao.writeRow2Db(item: neoAccount, sendError: { it in self.alertMate.showAlert(msg: it)} )
            ...
            Task {
                do {
                    ...
                    DispatchQueue.main.async { [self] in
                        ...
                        gachaCloudRecord = gachaCloudRecord.filter({ $0.id == TBDao.getDefaultAccount()?.gameInfo.genshinUID ?? "" })
                    }
                } catch {
    ...
    func deleteCloudRecord() async {
        do {
            let result = try await TBHutaoService.deleteGachaRecord(
                uid: TBDao.getDefaultAccount()!.gameInfo.genshinUID, hutao: passport!
            )
