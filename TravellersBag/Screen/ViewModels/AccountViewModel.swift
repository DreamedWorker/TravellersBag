...
class AccountViewModel: ObservableObject {
    ...
    @MainActor func setDefault(account: MihoyoAccount) {
        if account.active {
            ...
            if let defAccount = TBDao.getDefaultAccount() {
    ...
    @MainActor func logoutFunc(account: MihoyoAccount) {
        ...
        TBDao.deleteRow4Db(item: account, sendError: { it in})
        getLocalAccounts()
        if accounts.count == 0 {
        ...
        do {
            ...
            do {
                ...
                DispatchQueue.main.async {
                    ...
                    TBDao.saveAfterChanges(sendError: { it in self.alertMate.showAlert(msg: it) })
    ...
    private func writeAccount(
        ...
        TBDao.writeRow2Db(item: newData, sendError: { it in self.makeAToastInIOThread(msg: it) })
    }
