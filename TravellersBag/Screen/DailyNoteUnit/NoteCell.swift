    ...
    struct NoteCell: View {
        ...
        var body: some View {
            VStack {
                //顶部标题栏
                HStack {
                    Text(account.gameInfo.genshinNicname).font(.title3).bold()
                    Text(
                        String.localizedStringWithFormat(
                            NSLocalizedString("dashboard.and", comment: ""), account.gameInfo.level, account.gameInfo.serverName))
                    .font(.callout)
                    Spacer()
            ...
            .alert(alert.msg, isPresented: $alert.showIt, actions: {
                if useBtn {
                    Button("app.confirm", role: .destructive, action: { alert.showIt = false; useBtn = false; delete() })
                }
            })
        }
